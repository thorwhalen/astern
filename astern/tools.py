...
def _run_lens(store: Store, lens_name: str, session: dict, turns: list[dict], *, force: bool = False,
    ...
    if errors:
        ...
        return {"lens": lens_name, "action": p.action, "reason": p.reason, "n_findings": 0,
                "usage": usage, "failed": True, "errors": errors[:3]}
...
def _synced(store: Store, *, session_id: str | None, since_days: float | None,
            projects: str | None, max_sessions: int | None) -> list[tuple[str, dict]]:
    ...
    cutoff = (datetime.now(timezone.utc).timestamp() - since_days * 86400) if since_days else None
    rows = []
    for sid in store.sessions:
        s = store.sessions[sid]
        if keep and not any(k in (s.get("project") or "") or k in (s.get("project_slug") or "")
                            for k in keep):
            continue
...
def judge(
    ...
    *,
    ...
    for sid, session in _synced(store, session_id=session_id, since_days=since_days,
                                projects=projects, max_sessions=max_sessions):
        turns = list(store.turns[sid]) if sid in store.turns else []
        entry = _ledger.get_entry(store.ledger, sid)
        p = _ledger.plan(entry, lens=lens, version=lens_.version, n_turns=len(turns),
                         incremental=lens_.incremental)
        row = {"session_id": sid, "title": session.get("title", ""),
               "project": session.get("project", ""), "n_turns": len(turns),
        ...
        if dry_run:
            ...
            view = _views.session_view(session, scope, prior=prior,
                                       **({"max_chars": max_chars} if max_chars else {}))
            feats = _estimate.features(session, scope, view)
            row.update({"view_chars": len(view), "features": feats,
        ...
        row.update({"seconds": round(time.time() - t0, 1), "usage": res.get("usage") or {},
                    "n_findings": res.get("n_findings", 0),
                    "failed": bool(res.get("failed")), "errors": res.get("errors"),
                    "view_chars": (rec.get("features") or {}).get("view_chars"),
                    "cost_usd": rec.get("cost_usd"), "model": rec.get("model"),
...
def _totals(rows: list[dict], *, dry_run: bool) -> dict:
    ...
    tot = {"n_sessions": len(rows), "n_skipped": len(rows) - len(acted), "n_run": len(acted),
...
def estimate(*, store: str | Store | None = None, lens: str = "synopsis",
             model: str = "haiku", home: str | None = None) -> dict:
    ...
    for sid, session in _synced(store, session_id=None, since_days=None, projects=None,
                                max_sessions=None):
        synced_ids.add(sid)
        turns = list(store.turns[sid]) if sid in store.turns else []
        p = _ledger.plan(_ledger.get_entry(store.ledger, sid), lens=lens, version=lens_.version,
                         n_turns=len(turns), incremental=lens_.incremental)
        if p.action == "skip":
            continue
        scope = turns[p.from_index:] if p.action == "incremental" else turns
        view = _views.session_view(session, scope,
                                   prior=_prior_of(store, lens, sid) if p.action == "incremental"
                                   else None)
        pending.append(_estimate.predict(cost_model, _estimate.features(session, scope, view)))
    ...
    corpus = _estimate.predict(cost_model, {"view_chars": 0, "bytes": unsynced_bytes / max(1, n_unsynced)})
    return {
        "store": str(store.root), "lens": lens, "model": model,
        "judged": {"n": cost_model["n"], "method": cost_model["method"],
                   "models": cost_model.get("models", [])},
        "cost_model": cost_model,
        "pending": {"n": len(pending), "proxy": "view (built from the store)",
                    **_sum_predictions(pending)},
        "corpus": {"n_sessions": n_corpus, "n_unsynced": n_unsynced, "bytes": unsynced_bytes,
                   "proxy": f"bytes × {per_byte:.4f} view chars/byte"
                            if per_byte else "bytes (no ratio measured yet)",
                   **_sum_predictions([corpus] * n_unsynced)},
    }


def _sum_predictions(preds: list[dict]) -> dict:
    return {"input_tokens": sum(int(p.get("input_tokens") or 0) for p in preds),
...
_dispatch_funcs = [sync, sessions, show, lenses, judge, estimate]
