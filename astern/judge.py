...
def _schema_note(schema: Mapping) -> str:
    ...
    return ("Respond with a single JSON object matching this JSON Schema, and nothing "
            f"else (no prose, no markdown, no code fences): {compact}")


def _command(*, claude_bin: str, model: str, effort: str | None, system: str | None,
            schema: Mapping | None, strict_schema: bool) -> list[str]:
...
def claude_judge(prompt: str, *, schema: Mapping | None = None, model: str = DFLT_MODEL,
                 ...
                 timeout_s: int = DFLT_TIMEOUT_S, claude_bin: str = "claude",
                 strict_schema: bool = False) -> Judgment:
    ...
    if schema is not None and not strict_schema:
        ...
        if system:
            ...
            prompt = f"{prompt}\n\n{note}"
    cmd = _command(claude_bin=claude_bin, model=model, effort=effort, system=system,
                   schema=schema, strict_schema=strict_schema)
    try:
