...
def _friction_report(fbs: dict, store: Store, top: int) -> tuple[dict, str]:
    ...
    for sid, findings in fbs.items():
        ...
        for f in findings:
            ...
            if kind == "tool_error":
                ...
                retry_digests[f"{ev.get('tool', '')}: {ev.get('digest', '')}"] += ev.get("attempts", 1)
        rows.append({"session_id": sid, "title": _title(store, sid), "n_findings": len(findings),
                     "kinds": dict(per_kind)})
    rows.sort(key=lambda r: r["n_findings"], reverse=True)
    agg = {"kind_counts": dict(kind_counts), "top_error_tools": tool_errors.most_common(top),
           "top_retry_digests": retry_digests.most_common(top), "sessions": rows[:top]}
    ...
    lines += [f"- {key} — {n} attempts" for key, n in retry_digests.most_common(top)] or ["- none"]
    lines += ["", "## Sessions ranked by friction", "", "| session | title | findings | top kind |",
              "|---|---|---|---|"]
    for r in rows[:top]:
        top_kind = max(r["kinds"], key=r["kinds"].get) if r["kinds"] else ""
        lines.append(f"| `{r['session_id'][:8]}` | {r['title']} | {r['n_findings']} | {top_kind} |")
    return agg, "\n".join(lines)
...
def _rewrites_report(fbs: dict, store: Store, top: int) -> tuple[dict, str]:
    ...
    for sid, findings in fbs.items():
        for f in _kinds_of(findings, "inline_script"):
            ...
            g = groups.setdefault(fp, {"first_line": ev.get("first_line", ""),
                                       "language": ev.get("language", ""), "sessions": set(), "n": 0})
            g["sessions"].add(sid)
            g["n"] += 1
    ...
    agg = {"n_scripts": n_scripts, "n_fingerprints": len(groups), "n_repeated_fingerprints": len(dup),
...
def _timeline_report(fbs: dict, store: Store, top: int) -> tuple[dict, str]:
    ...
    for sid, findings in fbs.items():
        for f in _kinds_of(findings, "session_timeline"):
            ev = f.get("evidence") or {}
            rows.append({"session_id": sid, "date": ev.get("ended_at", ""), "project": ev.get("project", ""),
                        "title": ev.get("title", ""), "prs": ev.get("prs") or [],
                        "n_files": len(ev.get("files_touched") or [])})
    rows.sort(key=lambda r: r["date"], reverse=True)
    agg = {"rows": rows[:top]}
    lines = [f"# Timeline ({len(rows)} sessions)", "", "| date | session | project | title | PRs | files |",
             "|---|---|---|---|---|---|"]
    for r in rows[:top]:
        prs = ", ".join(f"#{p.get('number')}" for p in r["prs"] if p.get("number")) or "-"
        lines.append(f"| {r['date']} | `{r['session_id'][:8]}` | {r['project']} | {r['title']} | "
                     f"{prs} | {r['n_files']} |")
    return agg, "\n".join(lines)
...
def _cost_report(fbs: dict, store: Store, top: int) -> tuple[dict, str]:
    ...
    for sid, findings in fbs.items():
        for f in _kinds_of(findings, "session_cost"):
            ...
            tokens = sum(v for u in usage_by_model.values() for k, v in u.items() if k != "api_calls")
            rows.append({"session_id": sid, "title": _title(store, sid), "usd": usd, "tokens": tokens})
            if isinstance(usd, (int, float)):
                total_usd += usd
            total_tokens += tokens
    rows.sort(key=lambda r: (r["usd"] or 0), reverse=True)
    agg = {"total_usd": round(total_usd, 4), "total_tokens": total_tokens, "sessions": rows[:top]}
    lines = [f"# Cost report ({len(fbs)} sessions)", "",
             f"Total: ${total_usd:.4f}, {total_tokens:,} tokens", "",
             "| session | title | USD | tokens |", "|---|---|---|---|"]
    for r in rows[:top]:
        usd = f"${r['usd']:.4f}" if isinstance(r["usd"], (int, float)) else "-"
        lines.append(f"| `{r['session_id'][:8]}` | {r['title']} | {usd} | {r['tokens']:,} |")
    return agg, "\n".join(lines)
...
def _hygiene_report(fbs: dict, store: Store, top: int) -> tuple[dict, str]:
    ...
    for sid, findings in fbs.items():
        for f in _kinds_of(findings, "hygiene"):
            rows.append({"session_id": sid, "title": _title(store, sid), **(f.get("evidence") or {})})

    def _median(key: str):
        vals = [r[key] for r in rows if isinstance(r.get(key), (int, float))]
        return statistics.median(vals) if vals else None

    medians = {k: _median(k) for k in ("read_edit_ratio", "write_share", "n_unread_edits",
                                       "tool_calls_per_turn")}
    rows.sort(key=lambda r: r.get("n_unread_edits", 0), reverse=True)
    agg = {"medians": medians, "sessions": rows[:top]}
    med_line = ", ".join(f"{k}={v:.2f}" for k, v in medians.items() if v is not None) or "n/a"
    lines = [f"# Hygiene report ({len(fbs)} sessions)", "", f"Fleet medians: {med_line}", "",
             "| session | title | read:edit | write share | unread edits | calls/turn | version |",
             "|---|---|---|---|---|---|---|"]
    for r in rows[:top]:
        re_ratio = f"{r['read_edit_ratio']:.2f}" if isinstance(r.get("read_edit_ratio"), (int, float)) else "-"
        ws = f"{r['write_share']:.2f}" if isinstance(r.get("write_share"), (int, float)) else "-"
        cpt = r.get("tool_calls_per_turn", 0) or 0
        lines.append(f"| `{r['session_id'][:8]}` | {r['title']} | {re_ratio} | {ws} | "
...
def _tooling_report(fbs: dict, store: Store, top: int) -> tuple[dict, str]:
    ...
    agg = {"top_tools": tools.most_common(top), "top_mcp_tools": mcp.most_common(top),
           "top_skills": skills.most_common(top), "top_subagents": subagents.most_common(top)}
    lines = [f"# Tooling report ({len(fbs)} sessions)"]
    for title, counter in (("Top tools", tools), ("Top MCP tools", mcp), ("Top skills", skills),
...
def _stats_report(fbs: dict, store: Store, top: int) -> tuple[dict, str]:
    ...
    for sid, findings in fbs.items():
        for f in _kinds_of(findings, "session_stats"):
            ev = f.get("evidence") or {}
            rows.append({"session_id": sid, "title": _title(store, sid), "n_turns": ev.get("n_turns", 0),
                        "n_tool_calls": ev.get("n_tool_calls", 0), "n_errors": ev.get("n_errors", 0),
                        "duration_ms": ev.get("duration_ms", 0)})
    rows.sort(key=lambda r: r["n_turns"], reverse=True)
    agg = {"sessions": rows[:top]}
    lines = [f"# Stats report ({len(fbs)} sessions)", "",
             "| session | title | turns | tool calls | errors | duration (s) |", "|---|---|---|---|---|---|"]
    for r in rows[:top]:
        lines.append(f"| `{r['session_id'][:8]}` | {r['title']} | {r['n_turns']} | {r['n_tool_calls']} | "
...
def _generic_report(fbs: dict, store: Store, top: int) -> tuple[dict, str]:
    ...
    lines += [f"- **{k}**: {v}" for k, v in kind_counts.most_common()] or ["- no findings"]
    return agg, "\n".join(lines)
...
def _build(lens_name: str, findings_by_session: dict, *, store: Store, top: int) -> tuple[dict, str]:
    reporter = _REPORTERS.get(lens_name, _generic_report)
    return reporter(findings_by_session, store, top)


def aggregate(lens_name: str, findings_by_session: dict, *, store: Store, top: int = 20) -> dict:
    """The JSON-able cross-session aggregate for one lens."""
    agg, _ = _build(lens_name, findings_by_session, store=store, top=top)
    return agg


def render(lens_name: str, findings_by_session: dict, *, store: Store, top: int = 20) -> str:
