...
def _user(uuid: str, text: str, *, ts: str) -> dict:
    return {"type": "user", "uuid": uuid, "sessionId": SID, "cwd": CWD, "timestamp": ts,
            "gitBranch": "main", "version": VERSION, "message": {"role": "user", "content": text}}


def _tool_result(uuid: str, tool_use_id: str, *, is_error: bool, content: str, ts: str) -> dict:
...
def _assistant(uuid: str, msg_id: str, *, blocks: list[dict], ts: str) -> dict:
    return {"type": "assistant", "uuid": uuid, "sessionId": SID, "timestamp": ts,
            "message": {"id": msg_id, "model": "claude-demo",
                       "usage": {"input_tokens": 5, "output_tokens": 5}, "content": blocks}}
...
def _system(subtype: str, *, ts: str, **extra) -> dict:
    return {"type": "system", "subtype": subtype, "sessionId": SID, "timestamp": ts, **extra}
...
def mk_records() -> list[dict]:
    ...
    return [
        ...
        {"type": "pr-link", "sessionId": SID, "prNumber": 42, "prUrl": "https://example/pr/42",
         "prRepository": "demo/demo"},
        {"type": "cost-state", "sessionId": SID, "totalCostUSD": 0.1234,
        ...
        _assistant("a0", "m0", ts="2026-09-01T00:00:01Z",
                   blocks=[_tool_use("t1", "Bash", {"command": "pytest -q"})]),
        _tool_result("u0r1", "t1", is_error=True, content="FAILED tests/test_x.py", ts="2026-09-01T00:00:02Z"),
        _assistant("a0b", "m0b", ts="2026-09-01T00:00:03Z",
                   blocks=[_tool_use("t2", "Bash", {"command": "pytest -q"}),
                          _text("Hmm, that's odd. Let me try that again.")]),
        _tool_result("u0r2", "t2", is_error=False, content="3 passed", ts="2026-09-01T00:00:04Z"),
        _assistant("a0c", "m0c", ts="2026-09-01T00:00:05Z", blocks=[_text("Tests are passing now.")]),
        _system("compact_boundary", ts="2026-09-01T00:00:06Z",
        ...
        _user("u1", "now count lines in a.py and record a helper script", ts="2026-09-01T00:01:00Z"),
        _assistant("a1", "m1", ts="2026-09-01T00:01:01Z",
                   blocks=[_tool_use("t3", "Edit", {"file_path": "/home/dev/proj/demo/a.py",
                                                    "old_string": "x", "new_string": "y"})]),
        _tool_result("u1r1", "t3", is_error=False, content="ok", ts="2026-09-01T00:01:02Z"),
        _assistant("a1b", "m1b", ts="2026-09-01T00:01:03Z",
                   blocks=[_tool_use("t4", "Write", {"file_path": "/tmp/scratch_count.py",
                                                     "content": "import sys\n"
                                                                "print(sum(1 for _ in open(sys.argv[1])))"})]),
        _tool_result("u1r2", "t4", is_error=False, content="ok", ts="2026-09-01T00:01:04Z"),
        _assistant("a1c", "m1c", ts="2026-09-01T00:01:05Z",
                   blocks=[_tool_use("t5", "Bash", {"command": "python3 - <<'EOF'\nprint('counting')\nEOF"})]),
        _tool_result("u1r3", "t5", is_error=False, content="counting", ts="2026-09-01T00:01:06Z"),
        _assistant("a1d", "m1d", ts="2026-09-01T00:01:07Z",
                   blocks=[_tool_use("t6", "ToolSearch", {"query": "select:Skill"})]),
        _tool_result("u1r4", "t6", is_error=False, content="ok", ts="2026-09-01T00:01:08Z"),
        _assistant("a1e", "m1e", ts="2026-09-01T00:01:09Z",
                   blocks=[_tool_use("t7", "Skill", {"skill": "wads-ci-fix"})]),
        _tool_result("u1r5", "t7", is_error=False, content="ok", ts="2026-09-01T00:01:10Z"),
